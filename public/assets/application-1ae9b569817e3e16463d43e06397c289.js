...
(function() {
  ...
  r = function() {
    return $('#new-product').click(function(e) {
      e.preventDefault();
      $('#new-product-form').fadeIn(500).removeClass('hide');
...
(function() {
  ...
  r = function() {
    var $change_picture, $cloudinary_fileupload, $drag_area, $drag_inner_div, $img_container, $profile_photo, $progress_bar, $upload_section, showLoading;
    ...
    $(window).load(function() {
      return $profile_photo = $('.profile-photo');
    });
    showLoading = function() {
      $drag_inner_div.toggleClass('hide');
      return $img_container.height($profile_photo.height());
    };
    $cloudinary_fileupload.bind('cloudinarydone', function(e, data) {
      $('.img').html($.cloudinary.image(data.result.public_id, {
        format: data.result.format,
        version: data.result.version
      }).addClass('img-responsive profile-photo'));
      $upload_section.hide();
      $drag_inner_div.toggleClass('hide');
      $('#user_image_id').val(data.result.public_id);
      return $('.profile-photo').load(function() {
        $img_container.height('');
        return $profile_photo = $('.profile-photo');
      });
    });
    $cloudinary_fileupload.bind('fileuploadprogress', function(e, data) {
      return $progress_bar.css('width', Math.round(data.loaded * 100.0 / data.total) + '%');
    });
    $drag_area.bind('dragover', function() {
      return $(this).addClass('hover');
    });
    $drag_area.bind('dragleave', function() {
      return $(this).removeClass('hover');
    });
    $drag_area.bind('drop', function() {
    ...
    return $change_picture.click(function(e) {
      e.preventDefault();
      $upload_section.css('position', 'absolute');
      $profile_photo.fadeOut(500);
      return $upload_section.fadeIn(500, function() {
